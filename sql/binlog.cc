...
class Binlog_cache_compressor {
 ...
 private:
  ...
  [[NODISCARD]] bool overwrite_cache_with_payload_event(
    ...
    if (m_cache.write_event(&m_thd, &tple) != 0) {
...
bool MYSQL_BIN_LOG::find_first_log_not_in_gtid_set(char *binlog_file_name,
  ...
  errmsg = ER_THD(current_thd, ER_SOURCE_HAS_PURGED_REQUIRED_GTIDS);
...
bool MYSQL_BIN_LOG::truncate_update_log_file(const char *log_name,
  ...
  if (valid_pos < binlog_size) {
    if (opt_trim_binlog) {
      ...
      fn_format(backup_file, "binlog_backup", opt_mysql_tmpdir, ".trunc", opt);
      ...
      if (my_copy(log_name, backup_file,
        ...
        // NO_LINT_DEBUG
        sql_print_error("Could not take backup of the truncated binlog file %s",
                        log_name);
      }
    }
...
void MYSQL_BIN_LOG::adjust_linfo_in_dump_threads(bool is_relay_log) {
  DBUG_TRACE;
  ...
  std::for_each(log_info_set.cbegin(), log_info_set.cend(),
                [is_relay_log](LOG_INFO *log_info) {
                  if (log_info->is_used_by_dump_thd) {
                    log_info->is_relay_log = is_relay_log;
                  }
                });
}
...
void MYSQL_BIN_LOG::adjust_linfo_offsets(my_off_t purge_offset,
  ...
  std::for_each(
      log_info_set.cbegin(), log_info_set.cend(),
      [purge_offset, is_relay_log](LOG_INFO *log_info) {
        if (!enable_raft_plugin || log_info->is_relay_log == is_relay_log) {
          /*
...
int MYSQL_BIN_LOG::open_binlog(const char *opt_name) {
  ...
  */
  if (!read_binlog_in_use_flag(binlog_file_reader)) {
    ...
    */
    error = ha_recover(&xids, nullptr, &engine_binlog_max_gtid, tmp_binlog_file,
                       &tmp_binlog_pos);
  ...
  bl_recovery.recover(&engine_binlog_max_gtid, engine_binlog_file,
...
void Dump_log::switch_log(bool relay_log, bool should_lock) {
  ...
  log_->adjust_linfo_in_dump_threads(relay_log);
