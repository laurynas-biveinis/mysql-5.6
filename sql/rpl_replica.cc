      ...
      if (mev->does_exist(binary_log::Metadata_event::Metadata_event_types::
        ...
        if (rli->last_opid.first != -1 && rli->last_opid.second != -1 &&
          ...
          rli->report(ERROR_LEVEL, ER_REPLICA_RELAY_LOG_READ_FAILURE,
                      ER_THD(thd, ER_REPLICA_RELAY_LOG_READ_FAILURE), msg);
          rli->abort_slave = 1;
          mysql_mutex_unlock(&rli->data_lock);
          delete ev;
          return 1;
...
bool mts_recovery_groups(Relay_log_info *rli) {
  ...
  */
  ...
  for (Slave_job_group *jg = above_lwm_jobs.begin(); jg != above_lwm_jobs.end();
    ...
    LogErr(DEBUG_LEVEL, ER_RPL_MTA_GROUP_RECOVERY_APPLIER_METADATA_FOR_WORKER,
  ...
  for (const auto &mismatch_info : thd->bi_mismatch_infos) {
    ...
    if (should_error_out || log_error_verbosity > 1) {
      ...
      if (!should_error_out) {
        ...
        msg << "Replica before-image consistency check failed at position: "
