...
// Here goes bulk_load_enable_unique_key_check
...
MYSQL_THDVAR_BOOL(blind_delete_primary_key, PLUGIN_VAR_RQCMDARG,
...
MYSQL_SYSVAR_STR(
    read_free_rpl_tables, read_free_rpl_tables,
...
MYSQL_SYSVAR_ENUM(
    ...
    "Control behavior when hitting I/O error. By default MyRocks aborts "
    "server "
...
MYSQL_THDVAR_BOOL(bulk_load_partial_index, PLUGIN_VAR_RQCMDARG,
                  "Materialize partial index during bulk load, instead of "
                  "leaving it empty.",
                  nullptr, nullptr, true);
...
MYSQL_THDVAR_ULONGLONG(
    ...
    "duration to sleep (in milliseconds) between calling chsize() to "
    "truncate "
...
MYSQL_SYSVAR_BOOL(create_if_missing,
...
MYSQL_SYSVAR_UINT(max_latest_deadlocks, max_latest_deadlocks,
                  ...
                  "deadlocks to store",
                  nullptr, set_max_latest_deadlocks,
...
MYSQL_SYSVAR_ENUM(
    info_log_level, info_log_level, PLUGIN_VAR_RQCMDARG,
    ...
    nullptr, set_rocksdb_info_log_level, rocksdb::InfoLogLevel::ERROR_LEVEL,
    &info_log_level_typelib);
...
MYSQL_SYSVAR_UINT(
    ...
    "DBOptions::wal_recovery_mode for RocksDB. Default is "
...
MYSQL_SYSVAR_ULONG(
    persistent_cache_size_mb, persistent_cache_size_mb,
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
MYSQL_SYSVAR_INT(
    ...
    "rocksdb_max_bottom_pri_background_compactions (e.g. 1 or 2), "
    "compactions "
...
MYSQL_SYSVAR_UINT64_T(wal_ttl_seconds,
                      get_rocksdb_db_options()->WAL_ttl_seconds,
                      PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
MYSQL_SYSVAR_BOOL(allow_mmap_writes,
...
MYSQL_SYSVAR_ENUM(
    index_type, index_type, PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
    "BlockBasedTableOptions::index_type for RocksDB", nullptr, nullptr,
    static_cast<unsigned long>(rdb_get_table_options().index_type),
    &index_type_typelib);
...
MYSQL_SYSVAR_BOOL(charge_memory, charge_memory,
                  PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_READONLY,
...
MYSQL_SYSVAR_UINT(flush_log_at_trx_commit, flush_log_at_trx_commit,
                  PLUGIN_VAR_RQCMDARG,
...
MYSQL_SYSVAR_BOOL(pause_ttl_compaction_filter, pause_ttl_compaction_filter,
                  PLUGIN_VAR_RQCMDARG,
...
MYSQL_SYSVAR_BOOL(force_flush_memtable_and_lzero_now,
                  force_flush_memtable_and_lzero_now_var, PLUGIN_VAR_RQCMDARG,
                  "Acts similar to force_flush_memtable_now, but also "
                  "compacts all L0 files.",
                  force_flush_memtable_and_lzero_now, rw_sysvar_update_noop,
                  false);
...
MYSQL_SYSVAR_UINT(
    seconds_between_stat_computes, seconds_between_stat_computes,
    PLUGIN_VAR_RQCMDARG,
...
MYSQL_SYSVAR_UINT(
    table_stats_sampling_pct, table_stats_sampling_pct,
    PLUGIN_VAR_RQCMDARG,
...
MYSQL_SYSVAR_UINT(
    stats_recalc_rate, stats_recalc_rate, PLUGIN_VAR_RQCMDARG,
...
MYSQL_SYSVAR_BOOL(
    allow_to_start_after_corruption, allow_to_start_after_corruption,
    PLUGIN_VAR_OPCMDARG | PLUGIN_VAR_READONLY,
...
MYSQL_SYSVAR_STR(
    trace_block_cache_access, block_cache_trace_options_str,
    PLUGIN_VAR_RQCMDARG | PLUGIN_VAR_MEMALLOC,
...
MYSQL_SYSVAR_UINT(select_bypass_rejected_query_history_size,
...
MYSQL_SYSVAR_UINT(
    select_bypass_debug_row_delay, select_bypass_debug_row_delay,
    PLUGIN_VAR_RQCMDARG,
...
MYSQL_SYSVAR_ULONGLONG(
    select_bypass_multiget_min, select_bypass_multiget_min, PLUGIN_VAR_RQCMDARG,
...
MYSQL_SYSVAR_UINT(
    max_intrinsic_tmp_table_write_count, max_intrinsic_tmp_table_write_count,
    PLUGIN_VAR_RQCMDARG,
...
MYSQL_SYSVAR_UINT(
    clone_checkpoint_max_age, clone_checkpoint_max_age, PLUGIN_VAR_RQCMDARG,
...
MYSQL_SYSVAR_ULONGLONG(
    converter_record_cached_length, converter_record_cached_length,
    PLUGIN_VAR_RQCMDARG,
    "Maximum number of bytes to cache on table handler for encoding table "
    "record data. 0 means no limit.",
    nullptr, nullptr, /* default */ converter_record_cached_length,
    /* min */ 0, /* max */ UINT64_MAX, 0);
...
// TODO(laurynas): return enum directly? but reference
ulong &manual_compaction_bottommost_level(THD *thd) {
  return THDVAR(thd, manual_compaction_bottommost_level);
}
