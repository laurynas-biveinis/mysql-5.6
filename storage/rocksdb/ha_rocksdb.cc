...
void rocksdb_set_max_bottom_pri_background_compactions_internal(uint val) {
...
int rocksdb_compact_column_family(THD *thd, const std::string &cf) {
  ...
  if (cfh != nullptr && rdb != nullptr) {
    const auto bottommost_level_compaction =
        static_cast<rocksdb::BottommostLevelCompaction>(
...
static int rocksdb_init_internal(void *const p) {
  ...
  if (rdb_has_wsenv()) {
    ...
    auto s = rocksdb::Env::LoadEnv(sysvars::wsenv_path, &ws_env);
    if (s.ok()) {
  ...
  std::string trash_dir = std::string(sysvars::rocksdb_datadir) + "/trash";
      ...
      for (uint j = 0; j < k->actual_key_parts; j++) {
        ...
        if (records > 0) {
          ...
          if ((x == REC_PER_KEY_UNKNOWN &&
            ...
            if (sysvars::debug_cardinality_multiplier == 2) {
              x = 1 << (k->actual_key_parts - j - 1);
            } else {
              x = 1;
              for (uint kp = 1; kp <= k->actual_key_parts - j - 1; kp++) {
                x *= sysvars::debug_cardinality_multiplier;
              }
            }
          }
...
ha_rows ha_rocksdb::records_in_range(uint inx, key_range *const min_key,
  ...
  */
  ...
  if (sysvars::debug_optimizer_n_rows > 0) {
    ret = sysvars::debug_optimizer_n_rows;
  } else if (ret == 0) {
    ret = 1;
  }
...
int ha_rocksdb::inplace_populate_sk(
  ...
  for (const auto &index : indexes) {
    ...
    if (index->is_partial_index() && !sysvars::bulk_load_partial_index(thd))
      continue;
...
void Rdb_manual_compaction_thread::run() {
  ...
  for (;;) {
    ...
    if (sysvars::debug_manual_compaction_delay > 0) {
